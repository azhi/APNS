  ...
  class Notification
    ...
    def self.packed_nofications(notifications)
      ...
      notifications.each do |notification|
        bytes << notification.packaged_notification_frame
      end

      bytes
    end

    def packaged_notification_frame
      # Each notification frame consists of
      # 1. (e.g. protocol version) 2 (unsigned char [1 byte])
      # 2. size of the full frame (unsigned int [4 byte], big endian)
      # 3. notification frame data
      pn = self.packaged_notification
      frame = [2, pn.bytesize].pack('CN') + pn
      frame
    end
